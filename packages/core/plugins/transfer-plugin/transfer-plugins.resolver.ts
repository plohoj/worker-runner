...
import { ITransferPluginPreparedData, ITransferPluginReceivedData, PLUGIN_CANNOT_TRANSFER_DATA, TransferPluginDataType } from './transfer-plugin-data';
...
export class TransferPluginsResolver implements Omit<ITransferPluginController, 'type'> {
    ...
    public transferData(
        ...
        for (const pluginController of this.pluginControllers.values()) {
            ...
            if (transferData !== PLUGIN_CANNOT_TRANSFER_DATA) {
    ...
    public cancelTransferData(
        ...
        for (const pluginController of this.pluginControllers.values()) {
            if (pluginController.cancelTransferData) {
                ...
                if (cancelResult !== PLUGIN_CANNOT_TRANSFER_DATA) {
    ...
    public receiveData(
        ...
        if (pluginController) {
            ...
            if (receivedData !== PLUGIN_CANNOT_TRANSFER_DATA) {
    ...
    protected registerPlugins(plugins: IPlugin[]): void {
